...
async function chatNode(state: AgentState, config?: RunnableConfig): Promise<Command> {
  ...
  try {
    ...
    const isQuotaError = errorMessage.includes('quota') || errorMessage.includes('429') || errorMessage.includes('InsufficientQuota');
    
    // Return an error message to the user
    const errorResponse = {
      role: "assistant" as const,
      content: isQuotaError 
        ? "抱歉，API配额已用完。请检查您的账户余额和计费设置。"
        : `抱歉，处理请求时发生错误：${errorMessage}`
    };
...
async function processStepsNode(state: AgentState, config?: RunnableConfig): Promise<Command> {
  ...
  // Use the same model configuration as chatNode to avoid quota issues
  const baseURL = process.env.DEEPSEEK_BASE_URL;
  const apiKey = process.env.DEEPSEEK_API_KEY;
  ...
  let finalResponse;
  try {
    finalResponse = await finalModel.invoke([
